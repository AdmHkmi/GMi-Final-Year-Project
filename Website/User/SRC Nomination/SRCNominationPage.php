...
// Check NominationVoteStatus of the signed-in user
...
if ($loggedInUser) {
    ...
    // Retrieve NominationVoteStatus for the logged-in user
    $stmt_src_status = $conn->prepare("SELECT NominationVoteStatus FROM VSStudents WHERE StudentID = ?");
    $stmt_src_status->bind_param("s", $loggedInUser);
    $stmt_src_status->execute();
    $stmt_src_status->bind_result($NominationVoteStatus);
    $stmt_src_status->fetch();
    $stmt_src_status->close();

    // Check if NominationVoteStatus equals 3 to hide the SRC Container
    if ($NominationVoteStatus >= 3) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['StudentID']) && $event_active) {
    ...
    if ($loggedInUser) {
        $check_vote_sql = "SELECT COUNT(*) AS count FROM VSCandidateVote WHERE StudentID = ? AND CandidateID = ?";
        ...
        if ($row_check_vote['count'] > 0) {
            ...
            try {
                ...
                // Increment NominationVoteStatus for the logged-in user
                $increment_status_sql = "UPDATE VSStudents SET NominationVoteStatus = NominationVoteStatus + 1 WHERE StudentID = ?";
                ...
                // Update TotalCandidateVote for the candidate in VSStudents table
                $update_total_vote_sql = "UPDATE VSStudents SET TotalCandidateVote = TotalCandidateVote + 1 WHERE StudentID = ?";
                ...
                // Update session variable with incremented NominationVoteStatus
                $_SESSION['NominationVoteStatus'] = isset($_SESSION['NominationVoteStatus']) ? $_SESSION['NominationVoteStatus'] + 1 : 1;

                // Check if NominationVoteStatus has reached 3 to hide the SRC Container
                if ($_SESSION['NominationVoteStatus'] >= 3) {
                    $hide_src_container = true;
                    $vote_message = "You have already casted all of your votes. Thank you for your participation.";
                } else {
    ...
    <?php if ($event_active): ?>
        <?php if (!$hide_src_container): ?>
            ...
            <script>
                ...
                $voted_candidates_sql = "SELECT CandidateID FROM VSCandidateVote WHERE StudentID = ?";
