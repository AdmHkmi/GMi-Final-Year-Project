...
if (isset($_SESSION['StudentID'])) {
    ...
    // Check if QRCode and Poster files exist and delete them if they do
    ...
    if ($checkResult->num_rows > 0) {
        ...
        if (!empty($existingData['QRCode'])) {
            $oldQRCodePath = $tempDir . $existingData['QRCode'];
            if (file_exists($oldQRCodePath)) {
                unlink($oldQRCodePath); // Delete old QR code image
            }
        }
        if (!empty($existingData['Poster'])) {
            $oldPosterPath = $tempDir . $existingData['Poster'];
            if (file_exists($oldPosterPath)) {
                unlink($oldPosterPath); // Delete old PDF poster
            }
        }
    }
