...
if ($loggedInUser) {
    ...
    // Retrieve CandidateVoteLimit for the logged-in user
    ...
    // Retrieve VoteLimit for the current event from VSEvents table
    $stmt_event_limit = $conn->prepare("SELECT VoteLimit FROM VSEvents WHERE EventName = 'Candidate Vote'"); // Corrected query
    $stmt_event_limit->execute();
    $stmt_event_limit->bind_result($EventVoteLimit);
    $stmt_event_limit->fetch();
    $stmt_event_limit->close();

    // Check if CandidateVoteLimit is greater than or equal to the event's VoteLimit
    if ($NominationVoteLimit >= $EventVoteLimit) {
