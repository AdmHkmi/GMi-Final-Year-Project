...
// Fetch the vote limit from VSEvents table
$conn_vote_limit = new mysqli($servername, $username, $password, $dbname);
if ($conn_vote_limit->connect_error) {
    die("Connection failed: " . $conn_vote_limit->connect_error);
}

$sql_event_limit = "SELECT VoteLimit FROM VSEvents WHERE EventName = 'Candidate Vote'"; // Adjust this query if needed
$result_event_limit = $conn_vote_limit->query($sql_event_limit);
if ($row = $result_event_limit->fetch_assoc()) {
    $nomination_vote_limit = $row['VoteLimit']; // Dynamic vote limit
}
...
$sql_vote_count = "SELECT CandidateVoteLimit FROM VSVote WHERE StudentID = ?";
...
if ($row = $result_vote_count->fetch_assoc()) {
    $current_vote_count = $row['CandidateVoteLimit']; // Fetch the dynamic vote count from the database
}
...
<head>
    ...
    <title>Candidate Vote Page</title>
...
<header class="header">
    ...
    <nav class="navbar">
        <ul>
            ...
            <li><a href="VoteCastingPage.php">Candidate Vote</a></li>
    ...
    <?php if ($event_active): ?>
        <?php if ($current_vote_count < $nomination_vote_limit): ?>
            <div class="Instruction">
                ...
                <b><i>You can only vote <?php echo $nomination_vote_limit; ?> times.</i></b>
            </div>
            <div class="Search">
            ...
            <div class="SRC-Container">
                ...
                $sql = "SELECT StudentID, StudentProfilePicture, StudentName FROM VSVote WHERE CandidateApproval = 1";
        ...
        <?php else: ?>
            <!-- Include ShowVoteHistory.php if the vote limit is reached -->
