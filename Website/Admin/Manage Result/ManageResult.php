...
<body>
    ...
    <main>
        ...
        <div class="instructions">
            <p>Welcome to the Manage Results page! Here, you can view existing results, share or unshare results, and reset candidates as needed.</p>
        </div>
        <div class="side-by-side-container">
        <div class="Container">
        <div class="header-section"><h2>Nomination Result</h2></div>
            ...
            <center>
                ...
                <div class="button-group">
                    ...
                    <form action="UnshareNominationResultProcess.php" method="post" onsubmit="return confirm('Are you sure you want to unshare the result?');">
                        <button type="submit" class="Unshare_Nomination_Result_Button">Unshare Result</button>
                    </form><br>
                    <form action="ShareNominationResultProcess.php" method="post" onsubmit="return confirm('Are you sure you want to share the result?');">
                        <button type="submit" class="Share_Nomination_Result_Button">Share Result</button>
                    </form><br>
                ...
                <?php
                    ...
                    echo "<p>Nomination Result Status: $statusText</p>";
        ...
        <p>View all approved Candidates. You can click 'Share' to share results with students, or 'Unshare' if you prefer not to share them. To change your decisions, click 'Reset.' Additionally, the 'Notify Student' button will send an email to inform students the result have been published.</p>
            <center>
                <h2>List of Approved Candidate</h2>
                <div class="button-group">
                    <form action="ResetSRC.php" method="post" onsubmit="return confirm('Are you sure you want to reset all approved Candidate?');">
                        <button type="submit" class="Reset_SRC_Button">Reset Candidate</button>
                    </form><br>
                    <form action="UnshareSRCResultProcess.php" method="post" onsubmit="return confirm('Are you sure you want to unshare the result?');">
                        <button type="submit" class="Unshare_SRC_Result_Button">Unshare Result</button>
                    </form><br>
                    <form action="ShareSRCResultProcess.php" method="post" onsubmit="return confirm('Are you sure you want to share the result?');">
                        <button type="submit" class="Share_SRC_Result_Button">Share Result</button>
                    </form><br>
                    <form action="EmailSRCResult.php" method="post" onsubmit="return confirm('Are you sure you want to notify the student about this Candidate result?');">
                        <button type="submit">Notify Students</button>
                    </form>
                </div>
                <?php
                ...
                // Fetch IsActive status from VSEvents table
                $fetchStmt = $conn->prepare("SELECT IsActive FROM VSEvents WHERE EventName = 'Candidate Result'");
                $fetchStmt->execute();
                $fetchStmt->bind_result($isActive);
                $fetchStmt->fetch();
                $fetchStmt->close();
