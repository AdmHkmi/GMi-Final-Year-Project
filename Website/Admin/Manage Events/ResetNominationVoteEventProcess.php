...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ResetEvent'])) {
    ...
    $reset_event_sql = "UPDATE VSEvents SET StartDate = NULL, EndDate = NULL, VoteLimit = NULL WHERE EventID = ?";
    ...
    if ($stmt_reset_event->execute()) {
        ...
        // Prepare SQL statement to reset vote counts and nomination statuses for all candidates
        $reset_users_sql = "UPDATE VSVote SET TotalCandidateVote = 0, NominationVoteLimit = 0";
        if ($conn->query($reset_users_sql) === TRUE) {
            // Prepare SQL statement to delete candidate vote history
            ...
            if ($conn->query($delete_candidate_vote_sql) === TRUE) {
                ...
                // On failure, display the error message for deleting candidate votes
                echo "Failed to delete candidate votes: " . $conn->error;
            }
        } else {
            // On failure, display the error message for resetting vote counts and nomination statuses
